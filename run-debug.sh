...
echo "obotron  port ${PRD_OBOTRON_PORT}"
echo "obotron debug port ${PRD_OBOTRON_DEBUG_PORT}"
...
echo "PRD_OBOTRON_PORT = ${port}"
./mvnw clean  quarkus:dev -Ddebug=${PRD_OBOTRON_DEBUG_PORT} -Dquarkus.http.port=${port} -DskipTests=true -Dinfinispan.client.hotrod.server_list=10.123.123.123:11222 -Dinfinispan.client.hotrod.client_intelligence=BASIC
